...
# Start the Cloudflare tunnel daemon for public access
log_step_start "Starting Cloudflare tunnel daemon"
if [ -x "$PROJECT_DIR/tunnel-daemon.sh" ]; then
        echo "[TUNNEL] Starting tunnel daemon for public URL access..."
        (
                cd "$PROJECT_DIR"
                exec "$PROJECT_DIR/tunnel-daemon.sh"
        ) >"$PROJECT_DIR/tunnel-daemon.log" 2>&1 &
        TUNNEL_PID=$!
        echo "[TUNNEL] Tunnel daemon started (PID: $TUNNEL_PID)"
        disown "$TUNNEL_PID" 2>/dev/null || true
else
        echo "[TUNNEL] tunnel-daemon.sh not found, skipping..."
fi
log_step_end "Starting Cloudflare tunnel daemon"
